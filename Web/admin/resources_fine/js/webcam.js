...
window.webcam = {
	...
	swf_url: '../resources_fine/flash/webcam.swf', // URI to webcam.swf movie (defaults to cwd)
	shutter_url: '../resources_fine/sound/shutter.mp3', // URI to shutter.mp3 sound
	...
	get_html: function(width, height, server_width, server_height) {
		...
		if (!server_width) server_width = 1000;
		if (!server_height) server_height = 562;
	...
	get_movie: function() {
		...
		if (!this.loaded) return alert("Hata: Movie is not loaded yet");
		var movie = document.getElementById('webcam_movie');
		if (!movie) alert("Hata: Cannot locate movie 'webcam_movie' in DOM");
		return movie;
	...
	set_shutter_sound: function(enabled, url) {
		...
		this.shutter_url = url ? url : '../resources_fine/sound/shutter.mp3';
	},
